...
- **Dark-first UI** with `surface-950` background and `brand-*` (sky blue) accent
- **Industrial/utilitarian** aesthetic with font pairing: Syne (display) + DM Sans (body) + JetBrains Mono (code)
- **Table row flash** animation on real-time stock updates
- **Slide-in panel** for stock updates (no navigation break)
- **Sticky filters** for large product lists

---

## 🔧 Architecture Notes

- `productService.js` acts as an API abstraction layer – swap the mock with real REST/GraphQL endpoints
- Mock data includes a `_simulateRealTimeChange()` for polling simulation
- Offline queue persists in Redux (can be extended to `localStorage` for persistence across sessions)
- Role checks happen both in components (UI) and could be enforced at API layer
...
- ✅ CSV export
- ✅ Multi-warehouse support (field on products)
- ✅ Dark mode (default)
- ⬜ Barcode scanner (can integrate `@ericblade/quagga2`)
- ⬜ Unit tests (Jest + React Testing Library)
- ⬜ Audit log view (history already recorded per product)
